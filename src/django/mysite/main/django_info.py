...
def django_info(revenue, sector):
    ...
    with open(os.path.join(current_dir, "cluster_label_mappings.pkl"), "rb") as f:
        cluster_label_mappings = pickle.load(f)

    with open(os.path.join(current_dir, "sector_revenue_data.pkl"), "rb") as f:
        sector_revenue_data = pickle.load(f)

    for sector in input_sectors:
        if sector in clustered_sectors:
            with open(
                os.path.join(current_dir, f"{sector}_kmeans_model.pkl"), "rb"
            ) as file:
